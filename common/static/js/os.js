...
function share_button_click(link,ev){
	if(link=='QRCODE'){
		...
		var title = encodeURIComponent(document.title + (window.jQuery ? (': ' + jQuery('blog.css .entry-content').text().replace(/\r|\n|\t/g,'').replace(/ +/g,' ').replace(/<!--(.*)\/\/-->/g,'').substr(0,100)) : ''));
		var pic = '';
		window.jQuery && jQuery('#content > blog.css img').each(function(){pic+=(pic?'||':'')+encodeURIComponent(jQuery(this).attr('src'));});
